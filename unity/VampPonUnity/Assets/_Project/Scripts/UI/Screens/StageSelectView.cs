...
namespace VampPon.UnitySpike.UI.Screens
{
    public sealed class StageSelectView : MonoBehaviour
    {
            ...
            Func<string, AppFlowCommandResult> startStageCommand = null)
        {
            ...
            var panel = U46ScreenFactory.Panel(transform, "StageSelectPaperMap", new Vector2(.055f, .08f), new Vector2(.945f, .94f), VisualBatchAssetProvider.Prefer(VisualBatchAssetProvider.StageMapPaperBase, AppQualityAssetProvider.StageSelectMapPanel), AppQualityStyleTokens.PaperBase);
        ...
        private void BuildCatalog(Transform parent)
        {
            ...
            foreach (var entry in StageCatalog.Entries)
            {
                ...
                var title = U46ScreenFactory.Label(card.transform, "StageTitle", entry.DisplayName, 14f, Ink(), new Vector2(.25f, .42f), new Vector2(.95f, .94f), TextAlignmentOptions.Center, font);
