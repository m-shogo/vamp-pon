...
namespace VampPon.UnitySpike.UI.Screens
{
    public sealed class ResultView : MonoBehaviour
    {
        ...
        public void Show(RunResultSnapshot snapshot)
        {
            ...
            Stat(0.08f, "時間", vm.ElapsedTimeLabel);
            Stat(0.51f, "影払い", vm.DefeatedEnemyLabel);
            Stat(0.08f, "記憶の欠片", vm.FragmentLabel, 0.61f);
            Stat(0.51f, "到達", vm.ReachedLevelLabel, 0.61f);
            ...
            {
                U46ScreenFactory.Label(content, "RewardsEmpty", "今夜は持ち帰った記憶はありません", 13f, Silver(), new Vector2(0.08f, 0.36f), new Vector2(0.92f, 0.45f), TextAlignmentOptions.Left, font);
            }
            else
            {
                for (var i = 0; i < vm.RewardCards.Count && i < 3; i++)
                    Reward(i, vm.RewardCards[i]);
            }
