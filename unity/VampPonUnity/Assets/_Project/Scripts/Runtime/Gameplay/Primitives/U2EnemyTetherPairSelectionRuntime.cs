            ...
            out U2EnemyTetherPairSelectionResult result)
        {
            ...
            for (var firstIndex = 0; firstIndex < candidates.Count - 1; firstIndex++)
            {
                ...
                for (var secondIndex = firstIndex + 1; secondIndex < candidates.Count; secondIndex++)
                {
                    ...
                    var pairDistanceSquared = DistanceSquared2D(first.transform.position, second.transform.position);
                    ...
                    if (!found ||
                        ...
                        (combinedScore == selectedCombinedScore && pairDistanceSquared < selectedPairDistanceSquared))
                    {
                        found = true;
                        selectedFirst = first;
