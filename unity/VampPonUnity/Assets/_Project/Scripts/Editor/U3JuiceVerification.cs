...
{
    public static class U3JuiceVerification
    {
        private const string BootScenePath = "Assets/_Project/Scenes/Boot/Boot.unity";
        private const string ReportPath = "Logs/u3_juice_verification_report.txt";
        private const string StatePath = "Logs/u3_juice_verification_state.txt";
        ...
        public static void Run()
        {
            try
            {
                ...
                File.WriteAllText(StatePath, "u3_verifying");
                startedAt = EditorApplication.timeSinceStartup;
        ...
        {
            if (EditorApplication.timeSinceStartup - startedAt > 18.0)
            {
                Finish("Timed out.", 1);
                return;
            }
            ...
            if (!movementStarted)
            {
                ...
                player.SetVerificationMoveInput(new Vector2(1f, 0.2f));
                controller.SpawnEnemyForVerification(playerObject.transform.position + new Vector3(1.15f, 0.15f, 0f));
                controller.SpawnEnemyForVerification(playerObject.transform.position + new Vector3(-1.2f, 0.4f, 0f));
                return;
            }

            if (!movementCleared && (Vector2.Distance(movementStartPosition, playerObject.transform.position) > 0.7f || elapsed > 3.4))
            {
            ...
            Finish(BuildReport(playerObject, controller, movementStartPosition), 0);
        }
