...
{
    public static class U13PrefabFlowScreenshotCapture
    {
        private const string OutputDirectory = "../../docs/design-targets/generated/unity-u13/screenshots";
        private const string ReportPath = "Logs/u13_prefab_flow_screenshot_report.txt";
        ...
        public static void Run()
        {
            try
            {
                ...
                {
                    log.Add(Capture(p, $"u13-result-prefab-candidate-{p.Width}x{p.Height}.png", "Assets/_Project/Prefabs/UI/Result/ResultRoot.prefab"));
                    log.Add(Capture(p, $"u13-stageselect-prefab-candidate-{p.Width}x{p.Height}.png", "Assets/_Project/Prefabs/UI/StageSelect/StageSelectRoot.prefab"));
                }

                log.Add(CaptureFlow(new Profile(390, 844), "u13-flow-map-proof-390x844.png"));
        ...
        private static string Capture(Profile profile, string fileName, string prefabPath)
        {
            return CaptureBuilt(profile, fileName, root =>
            {
        ...
        private static string CaptureFlow(Profile profile, string fileName)
        {
            return CaptureBuilt(profile, fileName, root =>
            {
                var flow = U13PrefabCandidateBuilder.BuildFlowMapProof();
                flow.transform.SetParent(root, false);
                var rect = flow.GetComponent<RectTransform>();
                rect.anchorMin = new Vector2(0.5f, 0.5f);
                rect.anchorMax = new Vector2(0.5f, 0.5f);
                rect.pivot = new Vector2(0.5f, 0.5f);
                rect.anchoredPosition = Vector2.zero;
        ...
        private static string CaptureBuilt(Profile profile, string fileName, Action<RectTransform> build)
        {
            var created = new List<UnityEngine.Object>();
            var camObj = new GameObject("U13Cam", typeof(Camera));
            created.Add(camObj);
            ...
            var canvasObj = new GameObject("U13Canvas", typeof(Canvas), typeof(CanvasScaler), typeof(GraphicRaycaster));
