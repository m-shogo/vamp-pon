# ヨルノシルベ Character Hub

Date: 2026-07-28  
Status: **CURRENT CHARACTER ENTRYPOINT**

> キャラクターに関する質問・設計・画像brief・会話・Bond・関係性を扱う時は、まずこの1ファイルを見る。  
> repo全体から毎回キャラ設定を探し直さない。移行済みlegacyは通常読まない。

---

# 1. 最初に読むCurrent master

| 知りたいこと | Current master |
| --- | --- |
| Current 21をすぐ理解する | `docs/character-book-v4.md` |
| 人生の核 / 矛盾 / 深い成長 / 黒耀化との接続 | `docs/character-deep-core-book-v1.md` |
| 日常 / 癖 / 怒り / 嘘 / 口調 / 呼び方 | `docs/CHARACTER-LIFE-AND-SPEECH.md` |
| 仲間同行 / Bond / 戦闘連携 / 不安定ペア | `docs/BOND.md` |
| 黒耀化 / 固有呼称 / もう一つの自分 | `docs/BLACK-YOUKA.md` |
| 誕生日 / 好物 / 趣味 | `docs/character-personal-profile-canon-v1.md` |
| 星座 / 星獣 / 由来 | `docs/character-star-beast-constellation-canon-v1.md` |
| 体型 / 年齢感 / 眼鏡 | `docs/character-silhouette-diversity-current-canon-v1.md` |
| 時代差 / 別時代の人物接続 | `docs/story-temporal-layer-and-character-connections-v1.md` |
| Current 21の繋がり候補 | `docs/character-connection-web-high-value-candidates-v1.md` |
| 不老の魔女 / 複数世代の弟子 / 恋愛史 | `docs/character-long-lived-witch-arc-v1.md` |
| production combat / art data | `src/game/data/characterCanon.ts` / `docs/180-unified-character-canon.md` |
| Core5灯合わせ | `src/game/data/pairLightArts.ts` |

通常の人物質問:

```txt
docs/CANON.md
↓
docs/CHARACTERS.md
↓
docs/character-book-v4.md
↓
必要なら detail master 1つ
```

`character-book-v3.md` はv4によりsuperseded。通常参照しない。

---

# 2. Current roster

## Core 5

**ユイ / アサ / ナギ / ミチル / トモリ**

## Circle 10

**セン / リツ / コヨリ / ゲン / ハナ / ユウビ / マドカ / シロ / トバリ / ネム**

## Shadow / Rival 5

**クロオリ / カナメ / カスミ / トキ / ツムギ**

Stable runtime IDs:

```txt
kuroori
kage1 = カナメ
kage2 = カスミ
kage3 = トキ
kage4 = ツムギ
```

## Official Reserve

**レン**

Current scope: **20 + reserve 1 = 21人**

---

# 3. 人物トーン

世界は暗い。
人物の性格は揃えない。

- 明るい
- ネガティブ
- 無口
- 強気
- 怖がり
- ギャル
- お嬢様
- 子ども
- ぽっちゃり
- 渋い大人

などが同居してよい。

全員に必要なのは「ポジティブ化」ではなく、**その人らしい成長**。

```txt
長所
↓
同じ長所が恐怖 / 焦りで弱点になる
↓
黒耀化では極端に噴き出す
↓
成長後は長所を捨てず扱い方を覚える
```

---

# 4. 同じ時代の人たちとは限らない

## USER DIRECTION

ヨルノシルベで出会う人物は、現実世界では必ずしも同時代ではない。

- 夜では会える
- 朝へ帰ればそれぞれの時代 / 人生へ戻る
- 長く夜へ関わる人物もいる
- 夜の滞在時間と現実時間は一致しなくてよい
- Exact yearや全員の年代はまだLOCKしない

重要:

> **人は別時代でも、物・言葉・記録は現実時間を順番に受け継げる。**

例:

```txt
トモリが昔ランタンを修理
↓
現実時間で受け継がれる
↓
後世のユイが持つ
↓
夜では二人が直接会う
```

全員を親族 / 昔からの知人にしない。

詳細: `docs/story-temporal-layer-and-character-connections-v1.md`

---

# 5. 五つの大きな問い

Core5とShadow5は善悪コピーではなく、同じ問いへの別回答。

| Core | Shadow | 問い |
| --- | --- | --- |
| ユイ — 戻す | クロオリ — 閉じる | 記憶は開くべきか |
| アサ — 名前を返す | カスミ — ぼかす | 名前は明かすべきか |
| ナギ — 封じる | カナメ — 盾になる | 守るとは何か |
| ミチル — 歩いて選ぶ | トキ — 測る | 正しい道は一つか |
| トモリ — 直す | ツムギ — 余白を残す | 壊れたものをどう残すか |

Circle10は、この大Themeを日常 / 小さな人生で証明する人々。

詳細: `docs/character-deep-core-book-v1.md`

---

# 6. Gameplay-first Bond

Bondは情報閲覧のための作業ではない。

```txt
Supportとして一緒に戦う
↓
Bondが育つ
↓
Personal / Support / Pair性能が強くなる
↓
別の組み合わせを試したくなる
↓
副作用として呼び方 / 敬語 / 日常 /人物情報も変わる
```

読む人は読む。
読まない人は「性能が上がってラッキー」でよい。

**仲が良い = 強い / 仲が悪い = 弱い**だけにはしない。

- 安定Pair: reliability / utility
- 対立Pair: high-risk / high-upside
- 片寄りPair: specific condition
- 禁忌Pair: hidden build candidate +明確なrun cost

詳細: `docs/BOND.md`

---

# 7. Star Beast

- 誕生日占いではない
- 好き / 共鳴する生物星座
- 星獣という小さなマスコット
- 原則重複なし
- 兄弟 / 親族 / 継承 / shared memory / hidden relation等の理由がある時だけ重複可

重要:

- ユイ = 獅子座
- ユイ × トモリ = 獅子座重複。真相未確定
- リツ × コヨリ = りょうけん座重複。兄妹理由

別時代設定により、重複理由は血縁だけでなく**同じ灯り / 技術 / 記憶 / 役目の継承**にもできる。

---

# 8. Silhouette diversity

- ぽっちゃり女性 = ハナ
- ぽっちゃり男性 = カナメ
- 渋い年長男性 = ゲン
- メガネ = シロ / レン
- 作業ゴーグル = トモリ

体型・年齢・眼鏡をギャグ記号にしない。

---

# 9. 黒耀化 — もう一つの自分

共通システム名は **黒耀化**。

全員が固有呼称を持つ方向。

- ユイ `黒灯化` は強く継承
- 他20人はWorking Naming Board
- Human Naming Review前にfinal lockしない

黒耀化は闇落ちでも外から入った別人格でもない。

> **本人の中に元からある願い / 力 / 恐怖が、未熟さや焦りで極端な形に到達した「もう一つの自分」。**

強い。
しかし本人らしさが強すぎて周囲が見えなくなる。

後で:

> 「あれは別人だった」

ではなく:

> **「あれも自分だった。扱い方を知らなかった。」**

と受け入れ、制御できるようになる。

詳細: `docs/BLACK-YOUKA.md`

---

# 10. Strong relationship directions

- **ユイ × アサ** — 主人公級バディ。恋愛なし
- **リツ × コヨリ** — 兄妹。互いに互いを救う。恋愛なし
- **ユイ × クロオリ** — 取り戻す vs 閉じて守る
- **ユイ × トモリ** — 同じ獅子。血縁 /継承 /別時代の同じ灯り等を候補として保持

一番重要:

> **ヨルノシルベで初めて会い、一緒に戦い、食べ、喧嘩し、助け合ったから仲間になった関係を残す。**

過去設定だけで全員を仲間にしない。

詳細: `docs/character-connection-web-high-value-candidates-v1.md`

---

# 11. Future high-priority characters

Current21へ自動追加しない。

- **ヒヨリ候補** — ギャル / 明るい肯定。皆を元気にしなければと思いすぎる
- **セリカ候補** — お嬢様 / 礼儀 /責任。頼ることが苦手
- **不老の魔女候補** — 長い時間・複数世代の弟子・友情・恋愛・結婚・家族・別離を経験
- **弟子候補** — 魔女の長い人物史の一人になり得るが、現在関係は未LOCK

### 不老の魔女

旧「弟子との恋愛なし」は撤回。

全弟子が恋愛対象ではない。
何百年もの人物史の中で:

- 純粋な師弟
- 家族同然
- 親友
- 恋人
- 結婚
- 子ども
- 別離
- 再会

など異なる人生を経験していることが深みになる。

人物の核:

> **人を愛することに飽きたのではない。愛した人が必ず自分より先へ行くことを知りすぎた人。**

夜では弟子が老いないため「現実より少し長く一緒にいられる場所」に見える。

黒耀化では:

> **誰も朝へ帰さなければ、誰も老いない。**

まで歪むCandidate。

詳細: `docs/character-long-lived-witch-arc-v1.md`

---

# 12. Current 21 Connection philosophy

全員を全員の親族 / 昔からの知人にしない。

関係は混ぜる。

1. 兄妹 / 親族 / 師弟 / 恋愛などの直接関係
2. 小物 / 記録 / 技術の継承
3. 思想の鏡
4. **ヨルノシルベで初めて作る関係**

4を必ず残す。

> **繋がっていたから仲間なのではなく、一緒に過ごしたから仲間になった。**

---

# 13. Legacy no-read policy

通常は:

```txt
docs/CANON.md
↓
docs/CHARACTERS.md
↓
docs/character-book-v4.md
↓
必要なら docs/character-deep-core-book-v1.md
↓
必要なdetail master 1つ
```

移行済みlegacy:
- `docs/legacy-design-migration-2026-07-28.md`

禁止:

- 毎回repo全体から設定を再発掘する
- migrated legacyをcurrent回答の根拠へ戻す
- `character-book-v3.md`をCurrent masterとして使う
- Current21とfuture candidateを混ぜる
- `カゲール1`〜`4`を人名として復活
- 誕生日由来の旧zodiacを星獣へ戻す
- supersededされた「不老の魔女 × 弟子は恋愛なし」を復活
- 深みを出すためだけに全員へ同じ悲劇 / 血縁 /記憶喪失を追加する
