...
2. `docs/unity-responsive-screen-policy.md` を確認する。
3. `docs/unity-ai-asset-production-rules.md` を確認する。
4. `docs/unity-roadmap-to-release.md` を確認する。
5. Unity HubでUnity 6 LTS patchを確認する。
6. `unity/VampPonUnity/` に2D URP projectを作る。
7. `Library/`, `Logs/`, `UserSettings/`, `.sln`, `.csproj` がgit管理外であることを確認する。
8. Boot / Stage1 sceneを作る。
9. 390x844 referenceと複数スマホ縦解像度で、SafeAreaCanvasとdark paper背景を確認する。
10. Yui placeholder、Ombu placeholder、lantern glow、EXP fragment吸引placeholderを入れる。
11. ここで初回U1 commitにする。
