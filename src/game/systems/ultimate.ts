...
export function updateUltimate(scene: Phaser.Scene, state: RuntimeState, dt: number): void {
  ...
  if (state.characterId === 'yui' && (state.berserk?.charge ?? 0) > 0) requestYuiExpressionRageSheet(scene);
  if (ult.activeRemaining > 0) ult.activeRemaining = Math.max(0, ult.activeRemaining - dt);
  if (!ult.ready) {
    ult.charge += dt;
  ...
  if (!state.ultimateRequested) return;
  state.ultimateRequested = false;
  if (!ult.ready || (state.berserk?.activeRemaining ?? 0) > 0) return;
...
function activateUltimate(scene: Phaser.Scene, state: RuntimeState): void {
  ...
  for (const frag of state.pickups) frag.magnetized = true;
  for (const e of state.enemies) {
    if (e.dead) continue;
    const def = enemyById.get(e.defId);
    if (eff.smallEnemyOnly && !(def?.tags.includes('small') ?? false)) continue;
    if (distance(e.x, e.y, p.x, p.y) > eff.radius) continue;
    const willDie = e.hp <= eff.damage;
    damageEnemy(scene, state, e, eff.damage);
    if (willDie && eff.dropBonus) spawnFragment(scene, state, e.x, e.y, eff.dropBonus);
  }
  ultimateFlash(scene);
