...
import { CHARACTER_DECISION_COMMITMENT_RESERVOIR } from './characterDecisionCommitmentReservoir.ts';
...
export const CHARACTER_AUTHOR_DB_RULES = {
  ...
  future15Required: 15,
  coverageDimensionCountRequired: 18,
  ...
  decisionCommitmentMayInferLeadershipMoralityOrAffection: false,
...
const leisureByStableId = byId(CHARACTER_LEISURE_PLAY_RESERVOIR);
const humorByStableId = byId(CHARACTER_HUMOR_TEASING_RESERVOIR);
const decisionByStableId = byId(CHARACTER_DECISION_COMMITMENT_RESERVOIR);
...
export const CHARACTER_AUTHOR_DB_COVERAGE = CHARACTER_AUTHOR_DB_IDENTITIES.map((identity)=>{
  ...
  return {
    ...
    coverage: {
      ...
      leisurePlay: Boolean(leisureByStableId.get(identity.stableProfileId)),
      humorTeasing: Boolean(humorByStableId.get(identity.stableProfileId)),
      decisionCommitment: Boolean(decisionByStableId.get(identity.stableProfileId)),
    ...
    sourceStatus: {
      ...
      decisionCommitment: 'AUTHOR_RESERVOIR_NON_CANON_NO_LOYALTY_MORALITY_SCORE',
...
export const characterAuthorDbCoverageSummary = {
  ...
  leisurePlayCoverage: CHARACTER_AUTHOR_DB_COVERAGE.filter((entry)=>entry.coverage.leisurePlay).length,
  humorTeasingCoverage: CHARACTER_AUTHOR_DB_COVERAGE.filter((entry)=>entry.coverage.humorTeasing).length,
  decisionCommitmentCoverage: CHARACTER_AUTHOR_DB_COVERAGE.filter((entry)=>entry.coverage.decisionCommitment).length,
