...
function addCutinAtmosphere(scene: Phaser.Scene, root: Phaser.GameObjects.Container, mode: CharacterCutinMode): void {
  ...
  const shadeColor = isBerserk ? STORYBOOK_UI.inkBlack : 0x071021;
...
function addBerserkAtmosphere(scene: Phaser.Scene, root: Phaser.GameObjects.Container): void {
  const edgeColor = STORYBOOK_UI.inkBlack;
  const accent = STORYBOOK_UI.dustyRose;
  const violet = STORYBOOK_UI.inkViolet;
  const lantern = STORYBOOK_UI.warmAmber;
...
function addImageCutin(
  ...
  const isBerserk = mode === 'berserk';
  const accent = isBerserk ? STORYBOOK_UI.dustyRose : 0xffcf70;
  const lantern = STORYBOOK_UI.warmAmber;
  ...
  const labelBack = scene.add.rectangle(86, y + CUTIN_BANNER_HEIGHT / 2 - 28, 144, 30, isBerserk ? STORYBOOK_UI.inkBlack : 0x2f2310, 0.78);
...
function addFallbackCutin(
  ...
  const isBerserk = mode === 'berserk';
  const accent = isBerserk ? STORYBOOK_UI.inkViolet : 0xd9b65f;
  const paper = isBerserk ? STORYBOOK_UI.deepNight : 0xeee1bd;
  ...
  const lanternGlow = isBerserk
    ? scene.add.circle(GAME_WIDTH - 120, panel.y + 36, 42, STORYBOOK_UI.warmAmber, 0.08).setBlendMode('ADD')
    : null;
...
function addFrontAccents(scene: Phaser.Scene, root: Phaser.GameObjects.Container, mode: CharacterCutinMode): void {
  const isBerserk = mode === 'berserk';
  const color = isBerserk ? STORYBOOK_UI.dawnPeach : 0xfff0b3;
  const lantern = STORYBOOK_UI.warmAmber;
