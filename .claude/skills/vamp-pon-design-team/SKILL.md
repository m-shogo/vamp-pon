# Vamp Pon Design Team Skill

Use this skill for visual asset direction and review.

## When to use

Use before creating or reviewing:

- avatar / player-side designs
- monsters
- items
- backgrounds
- UI screens
- icons
- cards
- graphic frames
- art direction boards

## Required docs

Read these first:

- `docs/design-team/README.md`
- `docs/design-team/art-director-rules.md`
- `docs/design-team/avatar-rules.md`
- `docs/design-team/monster-designer-rules.md`
- `docs/design-team/background-designer-rules.md`
- `docs/design-team/item-designer-rules.md`
- `docs/design-team/ui-ux-designer-rules.md`
- `docs/design-team/graphic-designer-rules.md`
- `docs/design-team/pro-app-quality-rubric.md`
- `docs/pixel-art/README.md`
- `docs/pixel-art/ng-patterns.md`

## Core rule

Every visual task must pass:

1. art direction fit
2. design-role fit
3. pixel-art craft check
4. gameplay readability check
5. production promotion gate when relevant

## Quality target

Do not accept 60-point work.
Target 80-point app quality before production promotion.

## Do not

- Do not rely on one-shot AI output.
- Do not use hair or color only for identity.
- Do not make black blob enemies.
- Do not make beautiful but noisy backgrounds.
- Do not make decorative UI that reduces readability.
- Do not promote without review evidence.
