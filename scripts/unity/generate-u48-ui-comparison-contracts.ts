...
const components: Record<string, Array<[string, string[]]>> = {
  hud: [
    ...
    ['hud-rare-slot', ['default', 'occupied', 'selected', 'disabled']],
  ...
  replacement: [
    ...
    ['replacement-cancel-button', ['default', 'pressed', 'disabled']],
  ],
  result: [
  ...
  stageSelect: [
    ...
    ['stage-select-metadata-row', ['default', 'disabled']],
  ],
};
...
const comparisonGroups = Object.entries(components).flatMap(([owner, entries]) => entries.map(([assetGroup, states]) => ({
  ...
  comparisonContract: {
    samePurpose: true,
      ...
      sameInteractionOwner: true,
      sameRequiredStates: states,
      sameContentContract: true,
      sameTapTargetContract: true,
  },
...
const value = {
  ...
  deprecatedGroups: [
    { assetGroup: 'ui-hud-inventory-frame', historyStatus: 'split-required-resolved', splitInto: comparisonGroups.filter(value => value.owner === 'hud').map(value => value.assetGroup) },
    { assetGroup: 'ui-levelup-card', historyStatus: 'split-required-resolved', splitInto: comparisonGroups.filter(value => value.owner === 'levelup').map(value => value.assetGroup) },
    { assetGroup: 'ui-replacement-modal', historyStatus: 'split-required-resolved', splitInto: comparisonGroups.filter(value => value.owner === 'replacement').map(value => value.assetGroup) },
    { assetGroup: 'ui-result-kit', historyStatus: 'split-required-resolved', splitInto: comparisonGroups.filter(value => value.owner === 'result').map(value => value.assetGroup) },
    { assetGroup: 'ui-stage-select-kit', historyStatus: 'split-required-resolved', splitInto: comparisonGroups.filter(value => value.owner === 'stageSelect').map(value => value.assetGroup) },
  ],
  comparisonGroups,
