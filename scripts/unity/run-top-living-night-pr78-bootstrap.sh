...
REPAIR="scripts/quality/repair-top-living-night-png-crc.mjs"
...
[[ -f "$REPAIR" ]] || fail "PNG CRC repair tool is missing: $REPAIR"
chmod +x "$RUNNER" "$REPAIR"

echo "Checking and repairing TOP PNG chunk CRCs ..."
node "$REPAIR"
...
if ! git diff --quiet -- "$KIT_ROOT/layers" "$KIT_ROOT/manifest.json"; then
  ...
  git commit -m "fix: repair TOP production PNG integrity"
  echo "Pushing repaired TOP assets to $SOURCE_BRANCH ..."
