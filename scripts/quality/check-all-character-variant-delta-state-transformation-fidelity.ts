...
if (!authority.includes('BASELINE_PRESERVING_MINIMUM_AUTHORIZED_DELTA')) fail('unknown state default missing');
...
for (const id of ids) {
  ...
  ], { cwd: root, encoding: 'utf8', maxBuffer: 88 * 1024 * 1024 });
  const output = JSON.parse(stdout);
  if (output.allCharacterVariantDeltaStateTransformationFidelityRequired !== true) fail(`${id}: variant fidelity flag missing`);
  for (const field of [
    'unknownStateDeltaMayBeInventedByImageModel',
    'stateMayRedesignIdentity',
    'stateMayChangeBodyCategory',
    'stateMayDeAgeCharacter',
    'stateMayIncreaseExposureAutomatically',
    'stateMayInventBodyModification',
    'stateMayRemoveMobilityEquipment',
    'stateMayResetMaintenanceHistory',
    'stateMayIncreaseOrnamentAutomatically',
    'stateMayReplaceIdentityPaletteAutomatically',
    'stateMayInventPropOrRelationshipEvidence',
    'premiumVariantMayBeautifyCharacterAutomatically',
    'generatedStateDeltaCreatesCanon',
  ]) if (output[field] !== false) fail(`${id}: variant guard weakened: ${field}`);
...
console.log(`[variant-delta-state] OK: ${ids.length}/36 final production prompts validated`);
