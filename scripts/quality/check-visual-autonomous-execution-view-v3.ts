...
const actualCounts = {
  totalManagedRows: current.counts?.totalManagedRows,
  currentAnnotatedRows: current.counts?.currentAnnotatedRows,
  imageBearingRows: current.counts?.imageBearingRows,
  currentImageBearingRows: current.counts?.currentImageBearingRows,
  logicalNonImageRows: current.counts?.logicalNonImageRows,
};
if (actualCounts.totalManagedRows !== actualCounts.currentAnnotatedRows || actualCounts.imageBearingRows !== actualCounts.currentImageBearingRows || Number(actualCounts.currentImageBearingRows) + Number(actualCounts.logicalNonImageRows) !== Number(actualCounts.totalManagedRows)) fail(`managed execution internal partition drifted: ${JSON.stringify(actualCounts)}`);
if (actualCounts.totalManagedRows !== runner.managedExecution?.stableRows || actualCounts.currentImageBearingRows !== runner.managedExecution?.imageBearingRows || actualCounts.logicalNonImageRows !== runner.managedExecution?.logicalNonImageRows) fail(`runner managed counts are stale: actual=${JSON.stringify(actualCounts)} runner=${JSON.stringify(runner.managedExecution)}`);
...
if (!Array.isArray(current.items) || current.items.length !== current.counts.totalManagedRows) fail('Current item count disagrees with managed count');
...
console.log(JSON.stringify({ status:'PASS', currentStage:runner.currentStage, actualCounts, settingsReady:true, characterMasterPromptSlots:144, bulkGameplayGenerationAllowedNow:false, logoExcluded:true }, null, 2));
