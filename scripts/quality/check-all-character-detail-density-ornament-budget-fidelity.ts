...
if (!authority.includes('MINIMUM_SUFFICIENT_AUTHORIZED_DETAIL')) fail('unknown detail default missing from authority');
...
for (const id of ids) {
  ...
  ], { cwd: root, encoding: 'utf8', maxBuffer: 68 * 1024 * 1024 });
  const output = JSON.parse(stdout);
  if (output.allCharacterDetailDensityOrnamentBudgetFidelityRequired !== true) fail(`${id}: detail-density flag missing`);
  for (const field of [
    'unknownDetailMayBeInventedByImageModel',
    'premiumAssetMayIncreaseOrnamentCountAutomatically',
    'highResolutionMayInventMicroDecoration',
    'importanceMayIncreaseOrnamentDensity',
    'rarityMayIncreaseOrnamentDensity',
    'ornamentMayCompensateForWeakIdentity',
    'ornamentMayAlterExposurePolicy',
    'ornamentMayAlterBodyModificationPolicy',
    'worldMotifMayBeRepeatedAsFiller',
    'generatedOrnamentCreatesCanon',
  ]) if (output[field] !== false) fail(`${id}: ornament guard weakened: ${field}`);
...
console.log(`[detail-density-ornament] OK: ${ids.length}/36 final production prompts validated`);
