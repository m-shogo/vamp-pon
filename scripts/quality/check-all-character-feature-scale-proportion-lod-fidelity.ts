...
if (!authority.includes('PROPORTION_PRESERVING_MINIMUM_SUFFICIENT_LOD')) fail('unknown LOD default missing from authority');
...
for (const id of ids) {
  ...
  ], { cwd: root, encoding: 'utf8', maxBuffer: 76 * 1024 * 1024 });
  const output = JSON.parse(stdout);
  if (output.allCharacterFeatureScaleProportionLodFidelityRequired !== true) fail(`${id}: LOD fidelity flag missing`);
  for (const field of [
    'unknownLodMayBeInventedByImageModel',
    'lodMayEnlargeEyesForReadability',
    'lodMayDeleteIdentityFaceLandmark',
    'lodMayBeautifyJawOrChin',
    'lodMayChangeHeadBodyRatio',
    'lodMayLengthenLegs',
    'lodMaySlimBodyCategory',
    'lodMayDeAgeCharacter',
    'lodMayShrinkMobilityEquipment',
    'lodMayMascotifyAnimal',
    'lodMayHumanizeRobot',
    'premiumAssetMayBeautifyProportionAutomatically',
    'generatedLodTreatmentCreatesCanon',
  ]) if (output[field] !== false) fail(`${id}: LOD guard weakened: ${field}`);
...
console.log(`[feature-scale-proportion-lod] OK: ${ids.length}/36 final production prompts validated`);
