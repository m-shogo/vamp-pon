...
if (entrypointPolicy.productionExporter !== CHARACTER_REFERENCE_PRODUCTION_ENTRYPOINT.exporter || CHARACTER_REFERENCE_PRODUCTION_ENTRYPOINT.policy !== entrypointPolicyPath) fail('official production entrypoint not promoted to v5 body-adornment wrapper');
...
console.log(`[body-adornment-topology] OK: ${ids.length}/36 final production prompts preserve source-backed body adornment and marking topology without invention`);
