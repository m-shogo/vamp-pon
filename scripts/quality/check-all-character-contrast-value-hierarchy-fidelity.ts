...
if (productionPolicy.productionExporter !== exporterPath) fail('value authority must use final surface production wrapper');
...
for (const id of ids) {
  const stdout = execFileSync(process.execPath, ['--experimental-strip-types', resolve(root, exporterPath), '--character', id, '--kind', 'character_reference'], { cwd: root, encoding: 'utf8', maxBuffer: 60 * 1024 * 1024 });
  ...
  for (const field of ['unknownValueHierarchyMayBeInventedByImageModel','contrastMayChangeSkinIdentity','contrastMayHideBodyCategory','contrastMayHideMobilityEquipment','contrastMayInventOutlineOrGlow','premiumAssetMayIncreaseContrastAutomatically','nightMayCrushIdentityMidtones','generatedValueTreatmentCreatesCanon']) {
    if (output[field] !== false) fail(`${id}: value guard weakened: ${field}`);
  }
...
console.log(`[contrast-value] OK: ${ids.length}/36 contrast/value production prompts validated`);
