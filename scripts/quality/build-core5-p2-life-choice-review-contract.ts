...
const p2Domains=Object.entries(queue.domainPriority??{}).filter(([,priority])=>priority==='P2').map(([domain])=>domain);
...
const characters=(profile.characters??[]).filter((character:any)=>core5.has(character.id)).map((character:any)=>({
  id:character.id,
  name:character.name,
  domains:Object.fromEntries(p2Domains.map(domain=>[domain,{
...
const contract={
  id:'yoru-no-shirube-core5-p2-life-choice-review-contract-v1',date:'2026-08-14',status:'DERIVED_REVIEW_CONTRACT_NON_CANON',
  sourceProfile:profilePath,sourceProfileSha256:createHash('sha256').update(profileText).digest('hex'),sourceQueue:queuePath,
  scope:{characterCount:characters.length,domainCount:p2Domains.length,reviewItemCount:characters.length*p2Domains.length},
...
if(process.argv.includes('--emit')){console.log('P2_CONTRACT_JSON_BEGIN');console.log(JSON.stringify(contract,null,2));console.log('P2_CONTRACT_JSON_END');}
else console.log(`[core5-p2-life-choice-review] OK: ${contract.scope.reviewItemCount} review items`);
