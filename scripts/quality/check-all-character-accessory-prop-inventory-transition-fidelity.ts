...
if (entrypointPolicy.productionExporter !== CHARACTER_REFERENCE_PRODUCTION_ENTRYPOINT.exporter || CHARACTER_REFERENCE_PRODUCTION_ENTRYPOINT.policy !== entrypointPolicyPath) fail('official production entrypoint not promoted to v6 accessory inventory wrapper');
...
console.log(`[accessory-prop-inventory] OK: ${ids.length}/36 final production prompts preserve source-backed removable object inventory and transitions without invention`);
