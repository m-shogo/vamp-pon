...
for (const token of [
  'IMPLEMENTED_SHARED_SELECTION_FOUNDATION',
  'CALLER_SUPPLIED_PROTOTYPE_TUNING_NOT_CANON',
  'local anchor',
  'TARGET_CHAIN_SELECTION',
  'Finite negative score',
  'no duplicate',
...
assert(currentUnityWeaponRuntimeCapabilities.TARGET_CHAIN_SELECTION === 'MISSING', 'foundation PR must not pre-promote TARGET_CHAIN_SELECTION capability');
assert(!coordinator.includes('U2EnemyTargetChainSelectionRuntime'), 'shared target chain selector must not enter live Stage1 coordinator');

console.log(JSON.stringify({
  status: 'PASS',
  primitive: 'TARGET_CHAIN_SELECTION_FOUNDATION',
