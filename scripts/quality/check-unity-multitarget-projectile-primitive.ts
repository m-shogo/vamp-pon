...
assert(emberSource.includes('battle.FireGameplayProjectilesAtNearestTargets('), 'Ember Matchcase must remain the Selected16-specific multi-target caller');
...
assert(title1BaseWeaponRuntimeAdmissionSummary.unityAdmittedRuntimeCount >= 1, 'multi-target primitive proof requires Ember implementation-review admission');
assert(title1BaseWeaponRuntimeAdmissionSummary.unityAdmittedWeaponIds.includes('ember_matchcase'), 'multi-target admission must continue to include Ember');
...
console.log(JSON.stringify({ status: 'PASS', deterministicNearestPrefix: true, equalDistanceTieBreak: 'pool-order', getInstanceIdDependency: false, selected16MultiTargetCaller: 'ember_matchcase', admittedPrototypes: title1BaseWeaponRuntimeAdmissionSummary.unityAdmittedWeaponIds, liveStage1Callers: 0 }, null, 2));
