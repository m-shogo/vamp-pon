...
for (const contract of contracts.contracts) {
  ...
  for (let index = 0; index < contract.goldenReferencePaths.length; index += 1) check(sha256(contract.goldenReferencePaths[index]) === contract.goldenReferenceSha256[index], `${contract.candidateId} golden hash ${index}`);
...
check(createHash('sha256').update(baselineProvider).digest('hex') === sha256(providerPath), 'production provider unchanged from Batch A baseline');
console.log('U48 Batch A contract check passed: 9 Golden Reference contracts, 36 pre-generation candidate contracts, deterministic recipes, and production provider isolation.');
