...
if (!authority.includes('IDENTITY_PRESERVING_NEUTRAL_BOUNDARY')) fail('unknown edge default missing from authority');
...
for (const id of ids) {
  ...
  for (const field of [
    'unknownEdgeMayBeInventedByImageModel',
    'lineWeightMayRedesignFace',
    'lineWeightMayRedesignBodyCategory',
    'smallScaleMayEnlargeEyesForReadability',
    'smallScaleMayInventOutlineForReadability',
    'premiumAssetMayBeautifyContourAutomatically',
    'lineCleanupMayHideMobilityEquipment',
    'lineCleanupMayInventExposure',
    'nonHumanBoundaryMayHumanize',
    'generatedEdgeTreatmentCreatesCanon',
  ]) if (output[field] !== false) fail(`${id}: edge guard weakened: ${field}`);
...
console.log(`[edge-line-shape-boundary] OK: ${ids.length}/36 final production prompts validated`);
