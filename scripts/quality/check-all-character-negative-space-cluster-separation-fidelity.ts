...
if (!authority.includes('CANDIDATE_REVIEW_REQUIRED') || !authority.includes('FUNCTIONAL_IDENTITY_PRESERVING_BREATHING_ROOM')) fail('authority boundary/default missing');
...
for (const id of ids) {
  ...
  for (const field of ['unknownSpacingMayBeInventedByImageModel','premiumAssetMayReduceNegativeSpaceAutomatically','effectsMayFillIdentityCriticalNegativeSpace','ornamentMayFillIdentityCriticalNegativeSpace','spacingMayIncreaseExposure','spacingMayInventGarmentCutout','spacingMayDetachPropFromUseRelation','spacingMayHideMobilityEquipment','relationshipMayForceTouchingSilhouette','generatedSpacingTreatmentCreatesCanon']) if (output[field] !== false) fail(`${id}: spacing guard weakened: ${field}`);
...
console.log(`[negative-space-cluster-separation] OK: ${ids.length}/36 production prompts validated through ${exporterPath}`);
