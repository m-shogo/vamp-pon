...
for (const path of [callerPath,metaPath,slamPath,knockbackPath,breakPath,statusRequestPath,coordinatorPath,contractPath,projectPath,docPath]) {
...
for (const token of [
  'public sealed class PavementHammerPrototypeTelemetry','public static class PavementHammerPrototypeRuntime',
  'public const string WeaponId = "pavement_hammer";','public const string ContentStatusId = "EXPOSED";',
  'CALLER_SUPPLIED_PROTOTYPE_TUNING_NOT_CANON','PROTOTYPE_CALLER_IMPLEMENTED_NOT_LIVE',
  'QUERY_DAMAGE_SURVIVING_STATUS_KNOCKBACK_BREAK_STAGGER','EnemyStatusRuntimeKind.Exposed',
  'U2EnemySlamWaveQueryRuntime.SelectTargets(','target.TakeDamage(damage, damageFlashSeconds)',
  'statusRequest.ApplyTo(target.Statuses)','U2EnemyKnockbackRuntime.TryApplyFromPoint(','U2EnemyBreakStaggerRuntime.TryApply(',
]) {
  assert(caller.includes(token), `Pavement Hammer prototype missing contract: ${token}`);
}
const orderTokens = ['U2EnemySlamWaveQueryRuntime.SelectTargets(','target.TakeDamage(damage, damageFlashSeconds)','statusRequest.ApplyTo(target.Statuses)','U2EnemyKnockbackRuntime.TryApplyFromPoint(','U2EnemyBreakStaggerRuntime.TryApply('] as const;
let previous = -1;
...
for (const forbidden of ['const float Damage','const float Break','const float Knockback','const float Stagger','DefaultDamage','DefaultBreak','DefaultKnockback','DefaultStagger','ParticleSystem','AudioSource','Camera.','Stage1GameplayRuntimeCoordinator','WeaponEffectType','LevelUp']) {
...
assert(unityPrototypeCallerImplementedWeaponIds.includes('pavement_hammer'), 'Pavement Hammer caller proof registry entry missing');
assert(hammer.prototypeCallerImplemented && hammer.missingUnityCapabilities.length === 0, 'Pavement Hammer primitive/caller proof incomplete');
...
for (const token of ['PavementHammerPrototypeRuntime','pavement_hammer','U2EnemySlamWaveQueryRuntime','U2EnemyBreakStaggerRuntime']) {
  assert(!coordinator.includes(token), `Pavement Hammer prototype leaked into live Stage1 coordinator: ${token}`);
}
for (const token of ['defeated target must not receive EXPOSED after damage','defeated target must not receive break/stagger driver','defeated target must not be knocked back after damage','surviving target must receive EXPOSED','first slam should accumulate sub-threshold break','second EXPOSED attempts should hit independent cooldown','second 60-point slam should cross 100-point TEST_ONLY threshold','threshold crossing should preserve 20 residual break','second knockback must apply before stagger anchor','invalid caller tuning must fail closed and clear scratch']) {
  assert(contract.includes(token), `Pavement Hammer executable contract missing scenario: ${token}`);
}
for (const linkedSource of ['PavementHammerPrototypeRuntime.cs','U2EnemySlamWaveQueryRuntime.cs','U2EnemyKnockbackRuntime.cs','U2EnemyBreakStaggerRuntime.cs','EnemyStatusRuntimeState.cs','EnemyStatusApplicationRequest.cs']) {
  assert(project.includes(linkedSource), `Pavement Hammer contract project must compile real source: ${linkedSource}`);
}
for (const token of ['PROTOTYPE_CALLER_IMPLEMENTED_NOT_LIVE','CALLER_SUPPLIED_PROTOTYPE_TUNING_NOT_CANON','QUERY_DAMAGE_SURVIVING_STATUS_KNOCKBACK_BREAK_STAGGER','ADMITTED_FOR_UNITY_IMPLEMENTATION_REVIEW','runtimeStatus = NOT_IMPLEMENTED','TEST_ONLY / NOT_CANON','Original / Canon boundary','runtimeAutoPromotionAllowed = false']) {
  assert(doc.includes(token), `Pavement Hammer prototype doc missing token: ${token}`);
}

console.log(JSON.stringify({ status: 'PASS', caller: 'PavementHammerPrototypeRuntime', admission: hammer.unityDecision, liveRuntimeStatus: hammer.runtimeStatus, liveStage1Changed: false, canonTuningChanged: false }, null, 2));
