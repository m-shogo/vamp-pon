...
for (const entry of CHARACTER_ERA_SCENE_SEEDS) {
  for (const [slot, value] of [['A', entry.dialogueA], ['B', entry.dialogueB]] as const) {
    ...
    if (normalized.length < 4) fail(`dialogue too thin: ${entry.id}/${slot}`);
...
console.log(`[character-era-dialogue-quality] OK 36 unique scene seeds / ${dialogueSeen.size} unique dialogue lines / no exact chronology exposition`);
