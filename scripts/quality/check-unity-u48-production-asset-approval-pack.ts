...
const batchBKeys = new Set(['ground-area-black-ink-bottle', 'ground-area-streetlamp-ring', 'ground-area-dawn-ink-lamp', 'kokuyou-charging', 'kokuyou-ready', 'kokuyou-active', 'kokuyou-recovery']);
const candidateSpecificKeys = new Set([...batchAKeys, ...batchBKeys]);
...
for (const group of manifest.assetGroups) {
  ...
  check(group.runtimeBaselineIsCandidateSpecific === candidateSpecificKeys.has(group.assetKey), `${group.assetKey} candidate-specific baseline boundary`);
  ...
  for (const candidate of group.candidates) {
    ...
    if (candidateSpecificKeys.has(group.assetKey)) {
      ...
      check(candidate.gameplaySizeReviewReady === true, `${candidate.candidateId} Batch A/B gameplay review ready`);
    } else {
...
console.log(`Unity U48 approval pack integrity check passed: 21 groups, ${candidateCount} unique candidate records, Batch A/B candidate-specific evidence present, ${shortageCount} groups below four candidates and ${blockedGroupCount} blocked groups remain; human approval and production readiness remain blocked.`);
